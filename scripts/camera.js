...
if (!("TextDetector" in window)) {
  document.querySelector("#camara section").innerHTML =
    "Detectar texto con tu cámara no está soportado en esta versión de la app para tu dispositivo. Es un servicio experimental disponible en Google Chrome para Android activando el flag 'Experimental Web Features'";
}
  ...
  try {
    ...
    if (texts.length == 0) {
      ...
      document.querySelector(
        "output"
      ).innerHTML = `${fullText} <a href='javascript:decir("${fullText}")'>📣</a>`;
    }
  } catch (e) {
    console.log(e);
